...
contract('Campaign', function ([deployer, manager, user]) {
  ...
  beforeEach(async () => {
    ...
    await factoryInstance.createCampaign(
      'test campaign',
      'description2',
      'http://somewhere.com',
      10000,
  ...
  it('manager be able to create plans', async function () {
    const id = 1212
    const idPro = 1313
    await campaignContract.createPlan(id, 2000, 'basic plan', false, {
      from: manager,
    })
    await campaignContract.createPlan(idPro, 4000, 'pro plan', true, {
      from: manager,
    })
  ...
  it('not manager should not be able to create plans', async function () {
    ...
    try {
      ...
      expect(errorPermission.message).to.include(
